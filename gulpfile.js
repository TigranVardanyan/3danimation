...
gulp.task('css-libs', function() {
    return gulp.src('app/css/libs.+(scss|sass)') // Выбираем файл для минификации
        .pipe(sass()) // Преобразуем Sass в CSS посредством gulp-sass
...
gulp.task('prebuild', async function() {
    ...
    var buildHtml = gulp.src('app/*.html') // html to production
        .pipe(gulp.dest('dist'));
    
    let buildPhp = gulp.src('app/*.php') // php to production
        .pipe(gulp.dest('dist'));
    
    var buildConfig = gulp.src('app/config/**/*.*') // config files
...
gulp.task('watch', function() {
    ...
    gulp.watch('app/*.+(html|php)', gulp.parallel('code')); // Наблюдение за HTML и PHP файлами в корне проекта
